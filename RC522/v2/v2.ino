...
void showWelcomeMessage() {
  ...
  oledDisplay.drawString(64, 24, "Aproxime o cartao");
  oledDisplay.display();
}
...
void loop() {
  // Reseta a mensagem após 3 segundos sem leitura
  if (lastReadTime != 0 && (millis() - lastReadTime >= 3000)) {
    showWelcomeMessage();
    lastReadTime = 0;
  }

  // Verifica comando do Python
  if (Serial.available() > 0) {
    String command = Serial.readStringUntil('\n');
    command.trim();
    if (command == "LER_RFID") {
      Serial.println("Comando recebido: LER_RFID"); // Depuração
      showWelcomeMessage();
      unsigned long startTime = millis();
      while (!mfrc522.PICC_IsNewCardPresent() || !mfrc522.PICC_ReadCardSerial()) {
        if (millis() - startTime > 5000) { // Timeout de 5 segundos
          Serial.println("Timeout: Nenhum cartao detectado");
          return;
        }
        delay(100);
      }
